...
void _main(void)
{
	...
	cprintf("\n%~[3] Allocate again [test first fit] [50%]\n");
	{
		...
		//cprintf("before if ptr_allocations[8] = %x\n",ptr_allocations[8]);
		if ((uint32) ptr_allocations[8] != (pagealloc_start + 1*Mega)) {
			//cprintf("inside if ptr_allocations[8] = %x\n",ptr_allocations[8]);
			//cprintf("inside if (pagealloc_start + 1*Mega) = %x\n", (pagealloc_start + 1*Mega));
			is_correct = 0;
		...
		if ((uint32) ptr_allocations[9] != (pagealloc_start + 4*Mega)) {
			//cprintf("inside if ptr_allocations[9] = %x\n",ptr_allocations[9]);
			//cprintf("inside if (pagealloc_start + 4*Mega) = %x\n", (pagealloc_start + 4*Mega));
			is_correct = 0;
			cprintf("Wrong start address for the allocated space... \n");}
		//if ((freeFrames - sys_calculate_free_frames()) != 256) { is_correct = 0; cprintf("Wrong allocation: \n");}
		if( (sys_pf_calculate_allocated_pages() - usedDiskPages) !=  0) { is_correct = 0; cprintf("Wrong page file allocation: \n");}
		...
		if ((uint32) ptr_allocations[10] != (pagealloc_start + 1*Mega + 512*kilo)) {
			//cprintf("inside if ptr_allocations[10] = %x\n",ptr_allocations[10]);
			//cprintf("inside if (pagealloc_start + 1*Mega + 512*kilo) = %x\n", (pagealloc_start + 1*Mega + 512*kilo));
		...
		if ((uint32) ptr_allocations[11] != (pagealloc_start + 8*Mega)) {
			//cprintf("inside if ptr_allocations[11] = %x\n",ptr_allocations[11]);
			//cprintf("inside if (pagealloc_start + 8*Mega) = %x\n", (pagealloc_start + 8*Mega));
			is_correct = 0; cprintf("Wrong start address for the allocated space... \n");}
		//if ((freeFrames - sys_calculate_free_frames()) != 256) { is_correct = 0; cprintf("Wrong allocation: \n");}
		if( (sys_pf_calculate_allocated_pages() - usedDiskPages) !=  0) { is_correct = 0; cprintf("Wrong page file allocation: \n");}
		if (is_correct)
		...
		if ((uint32) ptr_allocations[12] != (pagealloc_start + 14*Mega) ) {
			//cprintf("inside if ptr_allocations[12] = %x\n",ptr_allocations[12]);
			//cprintf("inside if (pagealloc_start + 14*Mega) = %x\n", (pagealloc_start + 14*Mega));
			is_correct = 0; cprintf("Wrong start address for the allocated space... \n");}
