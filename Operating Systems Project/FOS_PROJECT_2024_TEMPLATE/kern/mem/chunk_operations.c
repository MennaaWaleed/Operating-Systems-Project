...
void* sys_sbrk(int numOfPages)
{
	...
	//int found_mem = 1;
	if(size == 0 && numOfPages == 0) return (void*) env->u_sbrk;
	if(size > 0 && (env->u_sbrk + size) <= env->u_rlmt)
	{
		// allocate nothing??? no mapping
		// if no mem/ free frames are exhausted ???

		allocate_user_mem(env, env->u_sbrk, size);
		uint32 last_sbrk = env->u_sbrk;
		env->u_sbrk = env->u_sbrk + size;
		return (void*)last_sbrk;
	}
	return (void*)-1;
}
...
uint32 sys_get_perms(struct Env* e, uint32 va)
{
	uint32 *ptr_page_table = NULL;
	//uint32 *ptr_paged = (uint32*)e->env_page_directory;

	if(get_page_table(e->env_page_directory, va, &ptr_page_table) == TABLE_IN_MEMORY)
	{
		return pt_get_page_permissions(e->env_page_directory, va);
	}
	return 0x000;
}
...
void free_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	if (size == 0 || virtual_address == 0) return;
	...
	{
		//unmark page
		pt_set_page_permissions(e->env_page_directory, virtual_address , 0, M_BIT);
