...
struct WorkingSetElement* New_obj=(struct WorkingSetElement*)(kmalloc(sizeof(struct WorkingSetElement)));
	if(New_obj==NULL)
	 {
	   panic("failed to allocate mem for obj of WorkingSet");
	 }
	else
	 {
		New_obj->virtual_address=virtual_address;
