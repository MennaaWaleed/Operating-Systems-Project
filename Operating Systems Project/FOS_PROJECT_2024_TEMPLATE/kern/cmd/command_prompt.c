...
int process_command(int number_of_arguments, char** arguments)
{
	...
	LIST_INIT(&foundCommands);
	for(int idx=0;idx<NUM_OF_COMMANDS;idx++)
	{
		if(strcmp(commands[idx].name,arguments[0]) == 0)
		{
			if((number_of_arguments-1)==(commands[idx].num_of_args))
				return idx;
			else if((commands[idx].num_of_args == -1) && (number_of_arguments>0))
				return idx;
			else {
				LIST_INSERT_HEAD(&foundCommands, &commands[idx]);
	...
	for(int idx=0;idx<NUM_OF_COMMANDS;idx++)
	{
		int lenOFSub=strlen(arguments[0]),lenOfCommand=strlen(commands[idx].name);
		int ptrOfSub=0,ptrOfCommand=0;
			while(ptrOfSub<lenOFSub&&ptrOfCommand<lenOfCommand){
				if((arguments[0][ptrOfSub])==(commands[idx].name[ptrOfCommand])){
					ptrOfSub++;
				}
				ptrOfCommand++;
			}
		if(ptrOfSub==lenOFSub){
			SubsecFound=1;
			LIST_INSERT_HEAD(&foundCommands, &commands[idx]);
		}
	}
	if(SubsecFound)return CMD_MATCHED;
