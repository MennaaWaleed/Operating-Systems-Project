...
void env_set_priority(int envID, int priority)
{
	...
	struct Env* proc;
	int check = envid2env(envID, &proc, 0);

	//Your code is here
	//Comment the following line
	//panic("Not implemented yet");
	if(check != E_BAD_ENV && proc != 0)
	{
		proc->priority = priority;
		if(proc->env_status == ENV_READY)
		{
			// must remove from old q
			acquire_spinlock(&(ProcessQueues.qlock));
			cprintf("set_priority\n");
			sched_remove_ready(proc);    //REMOVE FROM THE PREVIOUS QUEUE BEFORE CHANGE PERIORITY
			release_spinlock(&(ProcessQueues.qlock));


			acquire_spinlock(&(ProcessQueues.qlock));
			sched_insert_ready(proc);   //INSERT TO THE NEW QUEUE AFTER CHANGING PERIORITY THIS FUNCTION HANDLE WHERE IT WILL BE IN THE PERIOTRIZED QUEUE
			release_spinlock(&(ProcessQueues.qlock));
		}
	}
	return;
