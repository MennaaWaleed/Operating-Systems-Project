...
struct Env* fos_scheduler_PRIRR()
{
	...
	//Comment the following line
	//panic("Not implemented yet");
	struct Env *env_cur = get_cpu_proc();//to get the p in cpu
	struct Env *env_next = NULL;

	if (env_cur != NULL)
	{
		sched_insert_ready(env_cur);
	}
	for (int i = 0 ; i < num_of_ready_queues ; i++)
	{
		struct Env_Queue *queue = &ProcessQueues.env_ready_queues[i];
		if(LIST_EMPTY(&(ProcessQueues.env_ready_queues[i]))) continue;
			if (LIST_LAST(queue) != NULL)
			{
				env_next = dequeue(queue);
				break;
			}
	}
	kclock_set_quantum(quantums[0]);
...
void clock_interrupt_handler(struct Trapframe* tf)
{
	...
	if (isSchedMethodPRIRR())
	{
		...
		for (int i = 0 ; i < num_of_ready_queues ; i++)
		{
			...
			{
				ptr_env->proc_ticks++;
				if (ptr_env != NULL && ptr_env->proc_ticks> threshStarv)
				{
					// cache priority from cur process
					int last_priority = ptr_env->priority;

					// update the priority of cur process
					if (last_priority == 0)
					{
						ptr_env->priority = 0;
					...
					else
					{
						...
						ptr_env->proc_ticks=0;
						ptr_env->priority = last_priority - 1;
						acquire_spinlock(&(ProcessQueues.qlock));
