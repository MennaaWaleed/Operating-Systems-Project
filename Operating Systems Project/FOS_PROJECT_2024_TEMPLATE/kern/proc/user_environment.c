...
//	panic("create_user_kern_stack() is not implemented yet...!!");
	...
	//On failure: panic
	void*Sttack_allocated=kmalloc(KERNEL_STACK_SIZE);
	if(Sttack_allocated==NULL)
	  {
		 panic("failed to allocate mem for stack");
	  }
	else
	{
		void*GUARD_PAGE=Sttack_allocated;
		pt_set_page_permissions(ptr_user_page_directory,(uint32)GUARD_PAGE,0,PERM_PRESENT);
		return Sttack_allocated;
	}
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
    ...
    e->u_start = ROUNDDOWN(daStart, PAGE_SIZE);
    e->u_sbrk = daStart; // ???
    e->u_rlmt = ROUNDUP(daLimit, PAGE_SIZE);
    if(e->u_sbrk > e->u_rlmt) return;
    initialize_dynamic_allocator(daStart, 0); // initial size of user block allocator should be 0
}
