...
/*******************************/
                    ...
                    // fun to validate user address
static inline int valid_address_memirna(uint32 addr) {
                                     // check for NULL pointer
    if (addr == 0) {
        return 0;
    }

    // check if address within space range
...
// fun to handle invalid addresses & exit user program
static inline void invalid_address_memirna() {
    env_exit();  // Exit the user program
}

void sys_free_user_mem(uint32 virtual_address, uint32 size)
{
	 // 1st validate virtual address
	    if (!valid_address_memirna(virtual_address) || !valid_address_memirna(virtual_address + size)) {
	    	invalid_address_memirna();
	        return;
	    }
	    // cont freeing mem after validation
...
void sys_allocate_user_mem(uint32 virtual_address, uint32 size)
{
	//TODO: [PROJECT'24.MS1 - #03] [2] SYSTEM CALLS - Params Validation
	 ...
	 // validate address range
	    ...
	    //// cont mem allocation after validation
	allocate_user_mem(cur_env, virtual_address, size);
	return;
}

uint32 sysfor_perms(uint32 virtual_address)
{
	if (!valid_address_memirna(virtual_address))
	{
		invalid_address_memirna();
		return 0;
	}

	return sys_get_perms(cur_env, virtual_address);//IN CHUNK
}
//TODO: definition of fun sys_env_set_priority
void sys_env_set_priority(int32 envID, int priority)
{
	env_set_priority(envID,priority);
}

void sys_enq(struct Env_Queue* queue, struct Env* env){
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_sbrk: //SYS_sbrk system call (expanding the heap)
		// we will use it in  allocate best or first fit
		// We need to cast it to uint32
		return (uint32)sys_sbrk(a1);  // Cast void* to uint32
		break;

	case SYS_free_user_mem:
		// call to free the allocated memory if we needed it
		sys_free_user_mem(a1, a2);
		return 0;
		break;

	case SYS_allocate_user_mem:
		// call to allocate memory if we needed it
		sys_allocate_user_mem(a1, a2);
		return 0;
		break;

	// TODO: add in switch case
	case SYS_get_perms:
		return sysfor_perms(a1);
		break;
	case SYS_sesperiority:
	    sys_env_set_priority((int32) a1, (int) a2);
	    return 0;
	    break;

//	    case SYS_enqueue:
//	    			sys_enq((struct Env_Queue*)a1);
//	    			break;
	    	case SYS_init_queue:
	    	...
	    	case SYS_dequeue:
	    		  ...
	    		  break;
	    	case SYS_enqueue:
	    		sys_enq((struct Env_Queue *)a1, (struct Env *)a2);
	    	    break;
	    	case SYS_sched:
	    			sys_sched();
	    		    break;
